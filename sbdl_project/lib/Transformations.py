...
#convert df into struct for final json format
...
#selecting and transforming required data from source: account's data
def get_contract(df):
...
#selecting and transforming required data from source: parties's data
...
#selecting and transforming required data from source: address's data
def get_address(df):
...
#joining the parties and address data
...
# joining the account with party-address joined data
def join_contract_party(c_df, p_df):
    return c_df.join(p_df, "account_id", "left_outer")

#adding header for final json data
