...
#defining the schema for all 3 source data file
...
def read_accounts(spark, env, enable_hive, hive_db):
    runtime_filter = ConfigLoader.get_filter(env, "account.filter")
    if enable_hive:
...
def get_parties(spark, env, enable_hive, hive_db):
    runtime_filter = ConfigLoader.get_filter(env, "parties.filter")
    if enable_hive:
...
def get_address(spark, env, enable_hive, hive_db):
    runtime_filter = ConfigLoader.get_filter(env, "address.filter")
    if enable_hive:
